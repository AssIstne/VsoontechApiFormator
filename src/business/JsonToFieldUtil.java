...
public class JsonToFieldUtil {
    ...
    public void run(PsiClass psiClass, String jsonString) {
        try {
            ...
            psiClass.addAfter(elementFactory.createCommentFromText("/** " + jsonString + " */", psiClass), psiClass.getLBrace());
        } catch (JSONException e) {
            // 忽略
        }
    }

    private void addObjectField(PsiClass psiClass, JSONObject jsonObject) {
        PsiElementFactory elementFactory = PsiElementFactory.SERVICE.getInstance(psiClass.getProject());
        ...
        mOverrider.overrideToString(psiClass);
    }

    private void addArrayField(PsiClass psiClass, JSONArray jsonArray) {
        PsiElementFactory elementFactory = PsiElementFactory.SERVICE.getInstance(psiClass.getProject());
    }
}
